...
const AccountPage: FC<AccountPageProps> = ({ className = "" }) => {
  ...
  const [profilePhoto, setProfilePhoto] = useState(currentUserData.profilePhoto)
  const [photoSrc, setPhotoSrc] = useState(currentUserData.photoSrc)

  const [userName, setUserName] = useState(currentUserData.userName)
  const [email, setEmail] = useState(currentUserData.email)
  const [aboutUser, setAboutUser] = useState(currentUserData.aboutUser)
  ...
  const takePhoto = (e: any) => {
    ...
    dispatch(userDataFetched({
      photoSrc: URL.createObjectURL(e.target.files[0])
    }))
  ...
  const uploadFile = () => {
          if(profilePhoto) {
            saveFile("photo.jpg", profilePhoto, {
              type: "image/png",
              onSuccess: (result: any) => {
                setUserData({photoSrc: result._url})
                dispatch(userDataFetched({
          ...
          <div className="flex flex-col md:flex-row">
            <div className="flex-shrink-0 flex items-start">
              <div className="relative rounded-full overflow-hidden flex">
                <Avatar profilePhoto={currentUserData.photoSrc} sizeClass="w-32 h-32" />
            ...
            <div className="flex-grow mt-10 md:mt-0 md:pl-16 max-w-3xl space-y-5 sm:space-y-6 md:sm:space-y-7">
              ...
              <div>
                ...
                <Textarea
                  rows={5}
                  className="mt-1.5"
                  onChange={(event: any) => handleChange(event, 'about') }
                  defaultValue={currentUserData.aboutUser}
              ...
              <div className="">
                ...
                <div className="mt-1.5 flex">
                  ...
                  <Input
                    className="!rounded-l-none"
                    onChange={(event: any) => handleChange(event, 'website') }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-2.5">
                <div>
                  ...
                  <div className="mt-1.5 flex">
                    ...
                    <Input
                      ...
                      onChange={(event: any) => handleChange(event, 'facebook') }
                      defaultValue={currentUserData.facebook}
                ...
                <div>
                  ...
                  <div className="mt-1.5 flex">
                    ...
                    <Input
                      ...
                      onChange={(event: any) => handleChange(event, 'twitter') }
                      defaultValue={currentUserData.twitter}
                      sizeClass="h-11 px-4 pl-2 pr-3"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1.5 flex">
                    ...
                    <Input
                      className="!rounded-l-none"
                      onChange={(event: any) => handleChange(event, 'telegram') }
                      placeholder="yourtelegram"
                      defaultValue={currentUserData.telegram}
                      sizeClass="h-11 px-4 pl-2 pr-3"
                    />
                  </div>
                </div>
              ...
              {/* ---- */}
              <div className="pt-2">
                <ButtonPrimary className="w-full" onClick={updateProfile}>Update profile</ButtonPrimary>
              </div>
            </div>
          </div>
        </div>
